...
## 19.8 Prerequisite supervision profile accepted by WP-FF-010

WP-FF-010 freezes a prerequisite-only `ff.ffmpeg-supervision@1` boundary for one audited file-output operation: exact-map AAC ADTS plus H264 Annex-B inputs are copied without re-encoding into a job-scoped MP4 output. The request binds absolute tool and file identities, executable content, normalized version/capability observations, the canonical operation projection, exact logical direct arguments, trusted environment bindings, protocol restrictions, resource claims, pipe credits, and deadlines. The public adapter requires a privately constructed validated invocation and independently re-correlates it to the request. `ff.ffmpeg-bound-runtime-invocation-canonical-json@1` separately binds the exact platform vector without changing the version-one logical request. This profile does not authorize arbitrary commands, transcoding, broad codec/container support, a shipped post-processing subsystem, or product progress.

Stdout is reserved for `-progress pipe:1`; stderr is a separate bounded diagnostic stream and is never parsed as progress. Both streams drain concurrently through fixed buffers with item, byte, allocation, parser-work, cadence, consumer-stall, tail, and wall-clock limits. Required progress evidence is never truncated. Diagnostic loss is explicit. Media is written only to the governed output file. Bounded ffprobe JSON is treated as untrusted input; selected source and output packet payloads are independently measured and canonicalized across ADTS/raw AAC and Annex-B/container H264 framing before output success is accepted.

The approved native boundary is a narrow Ferric-owned `windows-sys`/`libc` module under `FF-DEC-003`. Windows creates the direct child suspended with a pre-execution Job-list attribute and exact standard-handle list, verifies one active Job member before the exact single resume, sets `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, performs bounded whole-Job termination, caches the direct-child wait result, and independently queries `ActiveProcesses == 0`. The quiet `CREATE_NO_WINDOW` plus `-nostdin` profile declares graceful control unsupported; it does not claim CTRL_BREAK or stdin `q`. Numeric Windows exit status does not prove exception provenance, and a brokerless suspended-orphan interval remains residual uncertainty.

Linux uses an exact `libc` process-spawn boundary. Inputs are retained `O_NOFOLLOW` descriptors; output is exclusively precreated with `openat` and `O_NOFOLLOW` relative to the retained output-directory descriptor. FFmpeg receives only fixed audited descriptors through `-fd` and `fd:`; ffprobe receives the same retained media object. Path replacement cannot redirect media I/O after binding, while concurrent mutation through a writable descriptor that predates supervision remains outside the trusted-root threat model and is reported as residual uncertainty.

The Linux child also has a dedicated process group, bounded TERM then KILL escalation, cached direct-child `waitpid` typing, and an independent declared-group absence query. A controlled `setsid` counterexample must escape the group signal scope and then be cleaned up by the harness. Process groups are signal scopes, not hostile descendant containment, and direct-child wait never proves arbitrary descendant absence. Escaped descendants retaining pipe handles must cause bounded reader failure and poisoned capacity, never an unbounded join.

The supervisor reuses the existing atomic `ResourceVector`, `ResourceContractV1`, `OwnedResourceBroker`, `OwnedByteCreditBroker`, and `ByteCreditStage::FfmpegPipe` boundaries. Admissions and leases stay on the owning supervisor thread. Cancellation intent is sticky through spawn, control, exit, reap, diagnostics, validation, and release races; the absolute deadline is sampled before and after physical lease release while the release effect remains pending. Reaped zero, reaped nonzero, signal termination, opaque Windows termination, and wait failure are distinct outcomes; a successfully reaped child is never reaped again or force-killed. Diagnostics, output decision where applicable, and bounded cleanup precede explicit resource and byte-credit release.

Platform producers emit strict `ff.ffmpeg-platform-proof@2` reports. This is an exact reviewed transition from `ff.ffmpeg-platform-proof@1`: version 2 replaces the independently floored force-termination and direct-reap observations with an exact three-phase partition over adjacent shared-origin integer-millisecond boundaries, adds the scope-zero settlement duration and its explicit ceiling, and rejects version 1 rather than reinterpreting its frozen fields. The independent consumer recomputes request/operation, the platform-specific bound-runtime projection and exact arguments, fixture payload, limits, lifecycle, direct-wait, output-fact, diagnostic, progress, phase deadlines, and containment evidence from raw canonical fields, binds both reports to one clean Git source and common fixture contract/payload, and emits `ff.ffmpeg-cross-platform-proof@1`. Exact request and bound-runtime hashes remain platform-specific because OS, paths, identities, tool hashes, and the Linux descriptor vector are platform-bound; the cross-platform join uses the common operation, fixture, and limits projections. Missing platforms, dirty or mismatched source, forged/stale fields, false active-zero, missing direct wait, declaration-preserving behavior mutation, or missing Unix escape residual fail closed.

This accepted result remains prerequisite evidence. A later shipped consumer must independently re-prove its exact staged production artifact through `FF-GATE-RUNTIME-001`; WP-FF-010 cannot claim product, capability, runtime, packaging, release, or phase completion.
