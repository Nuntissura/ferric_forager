...
Every lifecycle transition is pure and owned. Invalid transitions leave state and trace unchanged. Traces are bounded and replayable. The commit/archive model enumerates effect-request and acknowledged durable-prefix states; an emitted intent never counts as success, and restart reconciliation never invents success from a partial prefix. Resource admission is one checked 13-dimensional vector transaction with exact grant ownership, bounded waiter item/byte counts, FIFO order among eligible waiters, a deterministic per-owner active-grant ceiling, explicit cancellation dispatch, transactional release, and exact ownership checks. Byte credits conserve capacity, reject durability beyond cumulative credited bytes, and track received, validated, written, and durable positions monotonically.
...
- Compare cancellation acknowledgements with the originating request ID and generation before retiring ownership.
